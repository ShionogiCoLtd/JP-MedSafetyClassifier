...
app.layout = dbc.Container(
    [
        html.Div([
            ...
            dcc.Interval(id='interval_wait', interval=200, disabled=True, n_intervals=0, max_intervals=0),
        ]),
...
@app.callback(
        output=dict(
            ...
            interval_n_intervals=Output('interval_wait', 'n_intervals'),
        ),
        inputs=dict(
...
def load_file(set_progress,
    ...
    output = {
        ...
        'interval_wait_disabled': True,
        'interval_n_intervals': 0,
    ...
    if triggered_id == 'upload_csv':
        logger.info(f'Upload a csv file: {list_of_names}')
...
@app.callback(
        ...
        inputs=dict(
            interval_wait=Input('interval_wait', 'n_intervals'),
...
def extract_data_to_show(interval_wait, raw_data, scores_predicted,
    ...
    if triggered_id == 'interval_wait' and scores_predicted:
        index_list = create_index_list(data_length=len(raw_data[data_args.col_id]),
                                         filter_info=filter_item_value)
        if index not in index_list:
            index = index_list[0]    
